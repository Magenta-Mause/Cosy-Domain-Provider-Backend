...
public class Route53Service {
    ...
    /**
     * Lists all DNS records in the hosted zone.
     * Handles pagination automatically to fetch all records.
     */
    ...
    /**
     * Lists only A records (filters out NS, SOA, and others).
     * Useful if you only want user-created subdomains.
     */
    public List<DnsEntry> listARecords() {
        return listAllRecords().stream()
                .filter(r -> RRType.A.toString().equals(r.type()))
                .toList();
    }

    private DnsEntry toDnsEntry(ResourceRecordSet r) {
        List<String> values = r.resourceRecords().stream()
    ...
    private String submitChange(ChangeAction action, String name, String ip) {
        ...
        ChangeResourceRecordSetsResponse response = client.changeResourceRecordSets(
